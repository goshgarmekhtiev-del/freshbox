    ...
    <section id="faq" className="py-24 bg-white reveal">
      <div className="container mx-auto px-4 md:px-8 max-w-4xl">
        <div ref={headerRef as React.RefObject<HTMLDivElement>} className={`text-center mb-16 reveal reveal-fade-in ${headerVisible ? 'reveal-visible' : ''}`}>
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold leading-tight tracking-tight text-brand-text">Есть вопросы?</h2>
          <p className="text-lg font-medium text-brand-text-soft leading-relaxed mt-4">Мы знаем ответы</p>
        </div>
        ...
        <div className="space-y-4">
          {FAQS.map((item, index) => (
            <div 
              ...
              className={`rounded-[--radius-card] transition-all duration-300 overflow-hidden border-2 reveal reveal-slide-up ${faqReveals[index].isVisible ? 'reveal-visible' : ''} ${openIndex === index ? 'bg-orange-50 border-brand-accent/30 shadow-[--shadow-soft] scale-[1.02]' : 'bg-brand-bg border-transparent hover:bg-gray-100'}`}
            >
              <button
              ...
              >
                <p className="text-lg font-medium text-brand-text leading-relaxed px-6 md:px-8 pb-8">
                  {item.answer}
                </p>
              </div>
            </div>
